...
| `.red/tmp/supervisors/fleet/supervisor.log.toonl` | Fleet supervisor firehose — append-only TOONL heartbeat records for the supervisor itself. Read with `tail -f .red/tmp/supervisors/fleet/supervisor.log.toonl \| tq -p toonl -o json -r .msg`. Readers sniff legacy JSONL and mixed files; the lane is never rewritten in place. |
