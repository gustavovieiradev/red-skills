...
- `/afk fleet logs --supervisor` - read the local structured supervisor lane as
  human prose. Use `--worker <id>` for one worker or `--all` for every worker
  merged with `[worker-id]` prefixes. Add `--follow` to stream appended records
  live. This is local read-only and never contacts GitHub.
