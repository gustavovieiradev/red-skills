...
2. **PID-file pre-check / live directive.** Read `.red/state/castle/afk-supervisor.pid`. If it exists and `kill -0 <pid>` succeeds, do not launch a second supervisor. Instead the bundle command writes `.red/state/castle/afk-supervisor.resize.json` as the live directive:
   ...
   After writing the directive, the launcher reads `.red/state/castle/afk-supervisor.state.json` and prints `fleet directive applied ...` when the heartbeat already echoes the requested target/runner/shrink-mode, or `fleet directive pending ...` while waiting for the next supervisor tick to apply it.
   
   Older launch guidance used to say a live supervisor is refused:
   ```
   ✗ fleet already running (supervisor pid=<pid>, log .red/state/castle/afk-supervisor.log).
