...
async function mkAttempt(root: string, { worker, issue, branch, reason, validation }: MakeAttempt): Promise<void> {
  const dir = join(root, "workers", worker, `${issue}`);
...
describe("attempt-ledger context matching (pure core)", () => {
  ...
  it("picks a bare issue directory across workers", () => {
    const best = highestAttempt(
      "/r",
      249,
      entries([
        { worker: "wA1B9", basenames: ["249"] },
        { worker: "wC7D2", basenames: ["250"] },
      ]),
    );
    expect(best).toEqual({ attempt: 1, dir: "/r/workers/wA1B9/249" });
  });

  it("ignores other issues and attempt-suffixed legacy dirs", () => {
    const best = highestAttempt(
      "/r",
      249,
      entries([
        { worker: "wA1B9", basenames: ["250", "249-a2"] },
...
describe("attempt-ledger next number (FS-backed)", () => {
  ...
  it("detects only bare issue directories, independent per issue", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wA1B9", issue: 249, branch: "afk-attempts/wA1B9/249-foo", reason: "BLOCKED: typecheck failed" });
    expect(await attemptLedgerNextNumber(root, 249)).toBe(2);

    await mkdir(join(root, "workers", "wC7D2", "249-a10"), { recursive: true });
    expect(await attemptLedgerNextNumber(root, 249)).toBe(2);

    await mkAttempt(root, { worker: "wA1B9", issue: 250, reason: "fail" });
    expect(await attemptLedgerNextNumber(root, 250)).toBe(2);
    expect(await attemptLedgerNextNumber(root, 249)).toBe(2);
  ...
  it("ignores junk dirs that match the shell glob but not the attempt scheme", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wC7D2", issue: 249 });
...
describe("attempt-ledger prev dir", () => {
  it("returns the bare issue directory", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wA1B9", issue: 249 });
    expect(await attemptLedgerPrevDir(root, 249)).toBe(join(root, "workers", "wA1B9", "249"));
  });
...
describe("attempt-ledger restart context", () => {
  it("carries the previous attempt number, branch ref, and failure reason", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wC7D2", issue: 249, branch: "afk-attempts/wC7D2/249-foo", reason: "stall-reaped" });
    const context = await attemptLedgerContext(root, 249);
  ...
  it("labels missing marker files instead of failing", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wE5F6", issue: 777 });
  ...
  it("keeps the branch but labels a missing reason on a partial attempt", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wG7H8", issue: 778, branch: "afk-attempts/wG7H8/778-baz" });
  ...
  it("preserves multi-line failure reasons verbatim", async () => {
    const root = await tmpRoot();
    await mkAttempt(root, { worker: "wI9J0", issue: 779, branch: "afk-attempts/wI9J0/779-qux", reason: "line one\nline two" });
