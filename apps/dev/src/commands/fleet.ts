...
import { reapStaleSupervisorState, resolveLiveSupervisorPid } from "../runtime/supervisor-state.js";
...
export async function stopFleet(root = process.cwd(), stdout: NodeJS.WritableStream = process.stdout): Promise<FleetStopResult> {
  ...
  const pid = supervisor.pid ?? await resolveLiveSupervisorPid(pidFile, isLivePid);
  if (!pid) {
    await sweepOrphans();
