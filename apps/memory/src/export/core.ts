...
export function exportVectorStatus(vector: VectorStatusReport): Record<string, unknown> {
  return {
...
export function edgeKey(edge: Pick<BridgeEdge, "from_rid" | "to_rid" | "label">): string {
...
export function confidenceValue(props: Record<string, unknown>): string | null {
  const value = props.confidence;
  return typeof value === "string" && value.trim() ? value : null;
}

export function auditSeal(props: Record<string, unknown>): string {
  const value = props.audit_seal ?? props.confidence;
  return typeof value === "string" && value.trim() ? value : "AMBIGUOUS";
}

export function confidenceBand(props: Record<string, unknown>): string | null {
  const value = props.confidence_band;
  return typeof value === "string" && value.trim() ? value : null;
}

export function semanticLane(props: Record<string, unknown>): "INFERRED" | "EXTRACTED" | "AMBIGUOUS" {
  const seal = auditSeal(props);
  if (seal === "INFERRED" || seal === "EXTRACTED") return seal;
  return "AMBIGUOUS";
}

export interface SemanticLaneSummary {
...
export function buildSemanticLaneSummary(nodes: StoredNode[], edges: ExportEdge[]): SemanticLaneSummary {
