...
interface RenderHtmlHelpers {
  buildSemanticLaneSummary: (nodes: StoredNode[], edges: ExportEdge[]) => SemanticLaneSummary;
  exportVectorStatus: (vector: VectorStatusReport) => Record<string, unknown>;
  confidenceValue: (props: Record<string, unknown>) => string | null;
  auditSeal: (props: Record<string, unknown>) => string;
  confidenceBand: (props: Record<string, unknown>) => string | null;
  semanticLane: (props: Record<string, unknown>) => "INFERRED" | "EXTRACTED" | "AMBIGUOUS";
  edgeKey: (edge: { from_rid: number; to_rid: number; label: string }) => string;
}
