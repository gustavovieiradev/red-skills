...
import {
  maintainVectorProjection as maintainVectorProjectionImpl,
  projectDocVector,
  projectNodeVector,
  searchVector as searchVectorImpl,
  vectorStatus as vectorStatusImpl,
  type VectorOpsContext,
} from "./vector-ops.js";
import {
  ALGORITHMS,
  DIRECTIONS,
...
export class MemoryStore {
  ...
  async upsertNode(node: MemoryNode): Promise<number> {
    ...
    if (existing != null) {
      await projectNodeVector(this.vectorOpsContext(), existing, node, false);
    ...
    await projectNodeVector(this.vectorOpsContext(), rid, { ...node, properties }, false);
  ...
  async upsertDoc(doc: MemoryDoc): Promise<number> {
    ...
    await projectDocVector(this.vectorOpsContext(), rid, doc, false);
    return rid;
  }
  ...
  async searchVector(query: string, limit = 20): Promise<SearchRow[]> {
    return searchVectorImpl(this.vectorOpsContext(), query, limit);
  }

  async maintainVectorProjection(opts: { strict?: boolean } = {}): Promise<VectorStatusReport> {
    return maintainVectorProjectionImpl(this.vectorOpsContext(), opts);
  }

  async vectorStatus(): Promise<VectorStatusReport> {
    return vectorStatusImpl(this.vectorOpsContext());
  }

  private vectorOpsContext(): VectorOpsContext {
    return {
      db: this.db,
      project: this.project,
      listNodes: () => this.listNodes(),
      listDocs: () => this.listDocs(),
      kvGet: <T = unknown>(key: string) => this.kvGet<T>(key),
      kvPut: (key: string, value: unknown, expireMs?: number) => this.kvPut(key, value, expireMs),
