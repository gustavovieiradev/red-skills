...
export interface VectorOpsContext {
  db: RedDB;
  project: string;
  listNodes: () => Promise<MemoryNodeWithRid[]>;
  listDocs: () => Promise<Array<MemoryDoc & { rid: number }>>;
  kvGet: <T = unknown>(key: string) => Promise<T | null>;
  kvPut: (key: string, value: unknown, expireMs?: number) => Promise<void>;
  kvDelete: (key: string) => Promise<void>;
}

type MemoryNodeWithRid = MemoryNode & { rid: number };
...
export async function searchVector(
  ctx: VectorOpsContext,
  query: string, limit = 20): Promise<SearchRow[]> {
  const provider = await vectorReadProvider(ctx);
  if (provider == null) {
    throw new Error("RED_MEMORY_VECTOR_PROVIDER is not configured");
  }
  if (isLocalVectorProvider(provider)) {
    return searchLocalVector(ctx, query, limit);
  }
  const r = await ctx.db.query(
    `SEARCH SIMILAR TEXT '${escapeLabel(query)}' COLLECTION ${COLLECTIONS.vectors} USING ${provider} LIMIT ${clampLimit(limit)}`,
  );
  return groundVectorSearchRows(ctx, r.rows);
}

async function searchLocalVector(
  ctx: VectorOpsContext,
  query: string, limit = 20): Promise<SearchRow[]> {
  const queryEmbedding = localEmbedding(query);
  const rows = (await listLocalVectorRecords(ctx))
    .map((record) => ({
      record,
  ...
  for (const { record, score } of rows) {
    ...
    if (hashToNodeRid.size === 0) {
      for (const node of await ctx.listNodes()) {
...
async function groundVectorSearchRows(
  ctx: VectorOpsContext,
 ...
 */
export async function projectNodeVector(
  ctx: VectorOpsContext,
  rid: number,
  node: MemoryNode,
  strict: boolean,
  ...
  if (isLocalVectorProvider(provider)) {
    await recordLocalVector(ctx, {
      ...
      project: node.properties.project ?? ctx.project,
      provider,
      updated_at: updatedAt,
      embedding: encodeLocalEmbedding(localEmbedding(text)),
    });
    await clearVectorFailure(ctx, rid);
    return "ready";
  }
  try {
    await ctx.db.query(
      ...
      text.length,
      COLLECTIONS.nodes,
      node.properties.project ?? ctx.project,
      provider,
      updatedAt,
    );
    await clearVectorFailure(ctx, rid);
    ...
    await recordVectorFailure(ctx, rid, failure);
 ...
 * explicit.
 */
export async function projectDocVector(
  ctx: VectorOpsContext,
  rid: number,
  doc: MemoryDoc,
  strict: boolean,
  ...
  if (isLocalVectorProvider(provider)) {
    ...
    await clearVectorFailure(ctx, docVectorFailureKey(rid));
    return "ready";
  }
  try {
    await ctx.db.query(
    ...
    const failure = classifyVectorFailure(err, textHash, updatedAt);
    await recordVectorFailure(ctx, docVectorFailureKey(rid), failure);
    if (strict) throw new Error(`vector projection ${failure.status}: ${failure.error}`);
    return failure.status;
  }
}

export async function maintainVectorProjection(
  ctx: VectorOpsContext,
  opts: { strict?: boolean } = {},
): Promise<VectorStatusReport> {
  const strict = opts.strict === true;
  for (const node of await ctx.listNodes()) {
    await projectNodeVector(ctx, node.rid, node, strict);
  }
  for (const doc of await ctx.listDocs()) {
    await projectDocVector(ctx, doc.rid, doc, strict);
  }
  const report = await vectorStatus(ctx);
...
export async function vectorStatus(ctx: VectorOpsContext): Promise<VectorStatusReport> {
  const nodes = await ctx.listNodes();
  const docs = await ctx.listDocs();
  const provider = await vectorReadProvider(ctx);
  const records = provider == null ? [] : await listVectorRecords(ctx, provider);
  const byTarget = latestVectorRecords(records);
  const failures = await Promise.all(nodes.map((node) => readVectorFailure(ctx, node.rid)));
  ...
  const docFailures = await Promise.all(
    docs.map((doc) => readVectorFailure(ctx, docVectorFailureKey(doc.rid))),
  );
...
async function listVectorRecords(
  ctx: VectorOpsContext,
  provider: string,
): Promise<VectorProjectionRecord[]> {
  const localRecords = await listLocalVectorRecords(ctx);
  try {
    const r = await ctx.db.query(`SELECT * FROM ${COLLECTIONS.vectors}`);
...
async function vectorReadProvider(ctx: VectorOpsContext): Promise<string | null> {
  ...
  return (await readLocalVectorIndex(ctx)).size > 0 ? LOCAL_VECTOR_PROVIDER : null;
}

async function listLocalVectorRecords(ctx: VectorOpsContext): Promise<LocalVectorProjectionRecord[]> {
  const indexed = await readLocalVectorIndex(ctx);
  const records = await Promise.all([...indexed.values()].map((key) => readLocalVectorKey(ctx, key)));
  return records.filter((record): record is LocalVectorProjectionRecord => record !== null);
}

async function readLocalVector(
  ctx: VectorOpsContext,
  sourceCollection: string,
  rid: number,
): Promise<LocalVectorProjectionRecord | null> {
  const raw = await ctx.kvGet(localVectorKey(sourceCollection, rid));
  if (raw == null) return null;
  return (typeof raw === "string" ? JSON.parse(raw) : raw) as LocalVectorProjectionRecord;
}

async function recordLocalVector(
  ctx: VectorOpsContext,
  record: LocalVectorProjectionRecord,
): Promise<void> {
  const targetRid =
    record.source_collection === COLLECTIONS.docs ? record.doc_rid : record.node_rid;
  if (!Number.isFinite(targetRid)) return;
  await ctx.kvPut(localVectorKey(record.source_collection, Number(targetRid)), record);
  const index = await readLocalVectorIndex(ctx);
  index.set(
    vectorTargetKey(record.source_collection, Number(targetRid)),
    localVectorKey(record.source_collection, Number(targetRid)),
  );
  await writeLocalVectorIndex(ctx, index);
}

async function readLocalVectorIndex(ctx: VectorOpsContext): Promise<Map<string, string>> {
  const raw = await ctx.kvGet(LOCAL_VECTOR_INDEX_KEY);
...
async function writeLocalVectorIndex(
  ctx: VectorOpsContext,
  index: Map<string, string>,
): Promise<void> {
  await ctx.kvPut(LOCAL_VECTOR_INDEX_KEY, Object.fromEntries(index));
}

async function readLocalVectorKey(
  ctx: VectorOpsContext,
  key: string,
): Promise<LocalVectorProjectionRecord | null> {
  const raw = await ctx.kvGet(key);
  if (raw == null) return null;
  return (typeof raw === "string" ? JSON.parse(raw) : raw) as LocalVectorProjectionRecord;
}

async function readVectorFailure(
  ctx: VectorOpsContext,
  key: number | string,
): Promise<VectorFailureRecord | null> {
  const raw = await ctx.kvGet(typeof key === "number" ? vectorFailureKey(key) : key);
  if (raw == null) return null;
  return (typeof raw === "string" ? JSON.parse(raw) : raw) as VectorFailureRecord;
}

async function recordVectorFailure(
  ctx: VectorOpsContext,
  key: number | string,
  failure: VectorFailureRecord,
): Promise<void> {
  await ctx.kvPut(typeof key === "number" ? vectorFailureKey(key) : key, failure);
}

async function clearVectorFailure(
  ctx: VectorOpsContext,
  key: number | string,
): Promise<void> {
  await ctx.kvDelete(typeof key === "number" ? vectorFailureKey(key) : key);
}
