...
- **0083** AFK single-source invariants — one owner module per critical invariant: **Trunk** (`plugins.dev.trunk`, default `main`; the configurable branch all agent work enters from and exits to, always read as fresh-fetched `origin/<trunk>`; landing preconditioned on local-trunk-ancestor-of-origin, fail-loud, never reset/stash; precedence lock > pin > trunk), **untouchable primary** (agents never mutate the primary checkout's branches or WIP — flow is origin → worktree → origin only), **liveness lane** (red-castle-owned substrate-only stream + container-aware process cross-check; every consumer reads one evaluator), **exit barrier** (saved = pushed; every terminal path salvage-commits + pushes through one barrier). Kills the "fix only guards one path" class *(refines 0030/0031; builds on 0054 arming switch, 0065 vitals; supersedes hardcoded-`main` assumptions across apps/dev)*
